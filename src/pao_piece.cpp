...
bool PaoPiece::isBasicMove(const std::map<Pos, std::shared_ptr<Piece>> &context, int x, int y) const
{
    if(x < 1 || x > 9 || y < 1 || y > 10)
        return false;
    ...
    int expectedCount = Algorithms::containsPiece(context, x, y) ? 1 : 0;
    if(x == this->x)
    {
...
const std::list<Pos> PaoPiece::getPossibleMoves(const std::map<Pos, std::shared_ptr<Piece>> &context) const
{
    std::list<Pos> result;
    // 暴力十字线型尝试
...
PaoPiece::PaoPiece(int x, int y, bool side)
    : Piece(x, y, setSide(PieceType::PAO, side))
{
